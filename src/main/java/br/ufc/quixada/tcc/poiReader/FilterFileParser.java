...
public class FilterFileParser {
	...
	public List<Filtro> parse() {
		...
		// Parse lines
		List<Filtro> filters = new ArrayList<Filtro>();
		Filtro[] filterLevels = new Filtro[MAX_INDENT + 1];
		int lineNumber = 0;
		for(String line : lines) {
			...
			// Make filter
			Filtro filter = new Filtro(key, value, category);
			...
			if(level == 0) {
				...
				// Reset filter levels
				filterLevels = new Filtro[MAX_INDENT + 1];
				filterLevels[0] = filter;
				continue;
			}
			
			// Get parent
			Filtro parent = filterLevels[level - 1];
			if(parent == null) {
	...
	private boolean isFilterValid(Filtro filter) {
